#!/usr/bin/env python3
"""TensorRT export + inference for W2V2AASIST. Self-contained (no shared package).
...
GPU, and exposes a drop-in `W2V2AASISTTRT` class identical to the PyTorch path except
the neural forward runs on TensorRT.

CLI:
  python trt_w2v2-aasist.py export   # ONNX -> engine -> parity -> sweep -> sidecar
  python trt_w2v2-aasist.py sweep     # re-run the batch sweep, update sidecar
  python trt_w2v2-aasist.py parity    # PyTorch vs TRT parity report
  python trt_w2v2-aasist.py score AUDIO.wav
...
# ======================= per-model config =======================
ENTRY_MODULE   = "w2v2_aasist"          # module exposing the AntiSpoofingModel subclass
ENTRY_CLASS    = "W2V2AASIST"          # the subclass name
SLUG           = "w2v2-aasist"
...
# ----------------------------------------------------------------------------
class W2V2AASISTTRT(_OrigClass):
